...
const generateAccessAndRefreshTokens = async (userId) =>{
     try{
       ...
       return {accessToken, refreshToken}
         ...
         throw new ApiError(
            500,
            "Something went wrong while generate",
...
const registerUser = asyncHandler(async(req,res)=>{
    ...
    const existedUser = await User.findOne({
     ...
     const user = await User.create({
       ...
       // generateTemporaryToken is expected to return the unhashed token,
       // the hashed token and the expiry — call it and consume the result
...
const refreshAccessToken = asyncHandler(async(req,res)=>{
      ...
      return res.status(200).cookie("accessToken", options).cookie("refreshToken", newRefreshToken, options).json(
        new ApiResponse(
          200, 
...
const forgotPasswordRequest = asyncHandler(async(req,res)=>{
  ...
  await sendEmail(
       ...
       return res.status(200).json(
          new ApiResponse(200, {}, "Password reset mail has been sent to your mail id")
       )
